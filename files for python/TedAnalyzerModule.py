...
column_name ​ column. If n is bigger than the number of rows in ​data ​ attribute, return all of the rows. 
    ...
    def get_unique_values_as_list(self, column_name):
        return self.get_data()[column_name].unique()
    """
    ...
    def get_unique_values_as_dict(self, column_name):
        lst_uni = self.get_unique_values_as_list(column_name)
        d = {lst_uni[x]: list(self.data[column_name].value_counts())[x] for x in range(len(lst_uni))}
        return d
    """
...
analyzer = TedAnalyzer("ted_main.txt")
analyzer.drop_na()
print(analyzer.get_na_counts())